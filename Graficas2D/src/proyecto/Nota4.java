...
public class Nota4 {
	...
	private BufferedImage buffer, nota4;
	private Graphics g;
	private Color borde = new Color(255,255,0);
	...
	public Nota4() {
		...
		nota4 = new BufferedImage(50, 50,BufferedImage.TYPE_INT_ARGB );
		g = (Graphics2D) nota4.createGraphics();
		...
		dibujarLineaVertical(x+R+R+12, y+R-3, y-R-R, borde);
		
		dibujarLinea(x+R+R+12, y-R-R, x+R+R+20, y-R-R-1,borde);
		dibujarLinea(x+R+R+12, y-R-R-3, x+R+R+24, y-R-R-4,borde);

		dibujarLineaVertical(x+R+R+24, y-R-R-4, y-R, borde);
		dibujarLineaVertical(x+R+R+21, y-R-R-1, y-R-2, borde);
		
		dibujarCircunferencia(x+R+R+21, y+R-5, R, 165, 455, borde);
		
		dibujarLinea(x+R, y-R-R, x+R+R+12, y-R-R-3, borde);
		dibujarLinea(x+R+R, y-R, x+R+12, y-R-3, borde);

		
		pintarInundacion(x+R, y+R, 1, new Color(nota4.getRGB(x+R, y+R)), amarillo);
	}
	...
	public void pintarInundacion(int x, int y, int d, Color c, Color fondo) {
	    ...
	    if (new Color(nota4.getRGB(x+1,y)).getRGB() == c.getRGB())pintarInundacion(x+1, y, d, c, fondo);
	    if (new Color(nota4.getRGB(x-1,y)).getRGB() == c.getRGB()) pintarInundacion(x-1, y, d, c, fondo);
	    if (new Color(nota4.getRGB(x,y+1)).getRGB() == c.getRGB()) pintarInundacion(x, y + 1, d, c, degradarColor(fondo, d));
	    if (new Color(nota4.getRGB(x, y - 1)).getRGB() == c.getRGB())  pintarInundacion(x,y-1,d, c, degradarColor(fondo, d));
	}
