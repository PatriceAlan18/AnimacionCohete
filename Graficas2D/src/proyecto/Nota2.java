...
public class Nota2 {
	...
	private BufferedImage buffer, nota2;
	private Graphics g;
	private Color borde = new Color(0,255,0);
	...
	public Nota2() {
		...
		nota2 = new BufferedImage(50, 50,BufferedImage.TYPE_INT_ARGB );
		g = (Graphics2D) nota2.createGraphics();
		
		dibujarCircunferencia(x+R, y+R, R, 165, 455, borde);
		dibujarLineaVertical(x+R+R, y-R-R-3, y+R, borde);
		dibujarLineaVertical(x+R, y, y-R-R-3, borde);
		dibujarLineaHorizontal(x+R, x+R+R, y-R-R-3, borde);
		pintarInundacion(x+R, y+R, 1, new Color(nota2.getRGB(x+R, y+R)), verde);
	}
	...
	public void pintarInundacion(int x, int y, int d, Color c, Color fondo) {
	    ...
	    if (new Color(nota2.getRGB(x+1,y)).getRGB() == c.getRGB())pintarInundacion(x+1, y, d, c, fondo);
	    if (new Color(nota2.getRGB(x-1,y)).getRGB() == c.getRGB()) pintarInundacion(x-1, y, d, c, fondo);
	    if (new Color(nota2.getRGB(x,y+1)).getRGB() == c.getRGB()) pintarInundacion(x, y + 1, d, c, degradarColor(fondo, d));
	    if (new Color(nota2.getRGB(x, y - 1)).getRGB() == c.getRGB())  pintarInundacion(x,y-1,d, c, degradarColor(fondo, d));
	}
